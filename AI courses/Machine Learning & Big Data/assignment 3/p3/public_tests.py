import numpy as np
import math
from logistic_reg import sigmoid, compute_cost, compute_gradient, compute_cost_reg, gradient_descent, compute_gradient_reg, predict
from utils import *
...
w_in, b_in, J_history = gradient_descent(X=X,y=y,alpha=0.001,b_in=-8,w_in=np.zeros(2),cost_function=compute_cost,gradient_function=compute_gradient,num_iters=10000,lambda_=0)
print([J_history[-1]])
...
plot_data(X=X, y=y)
plt.show()
w, b, J_history = gradient_descent(X=X,y=y,alpha=0.01,b_in=1,w_in=np.zeros(27),cost_function=compute_cost_reg,gradient_function=compute_gradient_reg,num_iters=10000, lambda_=0.01)
print([J_history[-1]])
